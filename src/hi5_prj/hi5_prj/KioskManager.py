...
class KioskManager(Node):
    def __init__(self, host, port):
        ...
        self.order_call_db_service = self.create_client(
            OrderCall, "order_call_db")  # db통신

        # ros통신 수신용 service
        self.order_status_service = self.create_service(
            OrderCall, "order_status", self.order_status_callback)  # hi5 수신
    ...
    def accept_clients(self):
        while self.running:
            ...
            client_thread = threading.Thread(
                target=self.handle_client, args=(self.conn, addr))
            client_thread.start()
        ...
        try:
            if req.data:
                ...
                self.get_logger().info(
                    f"11111111111111111cmd:{cmd}, status:{data}")
                if cmd == "OS":
                    # send to kiosk gui
